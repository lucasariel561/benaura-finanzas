...
st.markdown("""
    ...
    h1, h2, h3 { color: #5C4D42; font-family: 'Georgia', serif; }
    .css-1d391kg { background-color: #F3EBE1; }
    </style>
...
def get_connection(use_db=True):
    ...
    if use_db: config['database'] = DB_NAME
...
if menu == "📊 Dashboard General":
    ...
    if not df.empty:
        ...
        df['mes_anio'] = df['fecha'].dt.strftime('%B %Y').str.capitalize()
    ...
    if not df.empty:
        ...
        st.bar_chart(data=df_agrupado, x='Mes', y='Ganancia_Neta', color="#5C4D42")
    else:
    ...
    if not df.empty:
        ...
        if not pendientes.empty:
            st.dataframe(pendientes[['n_pedido', 'cliente', 'producto', 'entrega', 'estado']], use_container_width=True, hide_index=True)
            st.divider()
    ...
    if not df.empty:
        clientes_con_telefono = df[df['telefono'] != '']
